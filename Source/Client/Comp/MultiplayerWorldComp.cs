    ...
    public class MultiplayerWorldComp : IExposable, ITickable
    {
        ...
        public void ExecuteCmd(ScheduledCommand cmd)
        {
            ...
            try
            {
                ...
                if (cmdType == CommandType.Autosave) LongEventHandler.QueueLongEvent(DoAutosave, "MpSaving", false, null);
            }
            catch (Exception e)
            {
        ...
        private void ExposeFactionData()
        {
            ...
            if (Scribe.mode == LoadSaveMode.LoadingVars && Multiplayer.session != null && Multiplayer.game != null) Multiplayer.game.myFactionLoading = Find.FactionManager.GetById(Multiplayer.session.myFactionId);

            if (Scribe.mode == LoadSaveMode.PostLoadInit) factionData[currentFactionId] = FactionWorldData.FromCurrent(currentFactionId);
        }
        ...
        private static string AutosaveFile()
        {
            ...
            if (MultiplayerMod.settings.appendNameToAutosave) autosavePrefix += $"{Multiplayer.session.gameName}-";
